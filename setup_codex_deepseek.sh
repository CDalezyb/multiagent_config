...
GO_HELP
    ...
    if go build -o "$bin_path" ./cmd/moonbridge; then
        ...
        cd - >/dev/null
        # Go 1.25 在父目录存在 .git 时可能触发 VCS 检测导致失败。
        # 兜底：临时写 go.work，然后用 go build -C（不依赖 shell cd）。
        print_warn "直接构建失败，使用 go.work 覆盖 VCS 检测..."
        printf 'go 1.25.0\n\nuse .\n' > "$repo_dir/go.work"
        if ! go build -C "$repo_dir" -o "$bin_path" ./cmd/moonbridge; then
